require 'minitest/autorun'
require 'minitest/pride'
require_relative '../lib/transaction_repository.rb'

class TransactionRepositoryTest < Minitest::Test

  def setup
    @transaction_repository = TransactionRepository.new
  end

  def test_it_exists
    assert @transaction_repository
  end

  def test_it_has_an_all_method
    assert @transaction_repository.all
  end

  def test_find_by_all_method_returns_all_records
    transaction_repository = TransactionRepository.new('./test/support/transactions_test_two.csv')
    assert_equal ["ID: 1, INVOICE_ID: 1, CREDIT_CARD_NUMBER: 4654405418249632, RESULT: success", "ID: 2, INVOICE_ID: 2, CREDIT_CARD_NUMBER: 4580251236515201, RESULT: failure"], transaction_repository.all
  end

  def test_it_has_a_random_method
    assert @transaction_repository.random
  end

  def test_it_has_find_by_id_method
    assert @transaction_repository.find_by_id
  end

  def test_it_can_find_a_transaction_by_id
    assert_equal "ID: 3, INVOICE_ID: 4, CREDIT_CARD_NUMBER: 4354495077693036, RESULT: success", @transaction_repository.find_by_id(3)
  end

  def test_it_has_a_find_by_invoice_id_method
    assert @transaction_repository.find_by_invoice_id
  end

  def test_find_by_id_method_returns_a_match
    assert_equal "ID: 3, INVOICE_ID: 4, CREDIT_CARD_NUMBER: 4354495077693036, RESULT: success", @transaction_repository.find_by_invoice_id(4)
    assert_equal "ID: 2, INVOICE_ID: 2, CREDIT_CARD_NUMBER: 4580251236515201, RESULT: failure", @transaction_repository.find_by_invoice_id(2)
  end

  def test_it_has_a_find_all_by_result_method
    assert @transaction_repository.find_all_by_result
  end

  def test_find_all_by_result_method_returns_all_matches
    assert_equal ["ID: 2, INVOICE_ID: 2, CREDIT_CARD_NUMBER: 4580251236515201, RESULT: failure", "ID: 6, INVOICE_ID: 7, CREDIT_CARD_NUMBER: 4580251236511201, RESULT: failure"], @transaction_repository.find_all_by_result("failure")
  end

end
