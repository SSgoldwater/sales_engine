...
class MerchantRepositoryTest < Minitest::Test
  ...
  def setup
    @merchant_repository = MerchantRepository.new("./test/support/merchants.csv", nil)
  end
  ...
  def test_all_method_returns_all_merchants
    expected_merchants = [Merchant.new({:name => "Schroeder-Jerde", :id => "1"}, nil), Merchant.new({:name => "Klein, Rempel and Jones", :id => "2"}, nil), Merchant.new({ :name => "Willms and Sons", :id => "3"}, nil ), Merchant.new({ :name => "Williamson Group", :id => "4"}, nil)]
  ...
  def test_it_has_find_by_name_method
    assert @merchant_repository.find_by_name
  end
